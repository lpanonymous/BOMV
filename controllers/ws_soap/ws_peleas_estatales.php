<?php
	...
	function agregarPeleaEstatal($categoria, $division, $id_juez1, $id_juez2, $id_juez3, $id_juez4, $id_boxeador1, $id_boxeador2, $fecha, $hora, $ganador, $foto, $nombre_foto){
		$conexion = mysqli_connect("localhost", "root", "", "torneo_box_olimpico");
		$location = "..\\..\\resources\\images\\peleas_estatales\\".$nombre_foto;                               // Mention where to upload the file
		...
		$agregar = $conexion->query("INSERT INTO peleas_estatales(categoria, division, id_juez1, id_juez2, id_juez3, id_juez4, id_boxeador1, id_boxeador2, fecha, hora, ganador, nombre_foto) VALUES ('$categoria', '$division','$id_juez1', '$id_juez2', '$id_juez3', '$id_juez4', '$id_boxeador1', '$id_boxeador2', '$fecha', '$hora', '$ganador', '$nombre_foto')");
	...
	function editarPeleaEstatal($id, $categoria, $division, $id_juez1, $id_juez2, $id_juez3, $id_juez4, $id_boxeador1, $id_boxeador2, $fecha, $hora, $ganador, $foto, $nombre_foto){
		$conexion = mysqli_connect("localhost", "root", "", "torneo_box_olimpico");
		$location = "..\\..\\resources\\images\\peleas_estatales\\".$nombre_foto;                               // Mention where to upload the file
        $current = file_get_contents($location);                     // Get the file content. This will create an empty file if the file does not exist     
        $current = base64_decode($foto);                          // Now decode the content which was sent by the client     
        file_put_contents($location, $current);                      // Write the decoded content in the file mentioned at particular location      
		$editar = $conexion->query("UPDATE peleas_estatales SET categoria='$categoria',division='$division',id_juez1='$id_juez1',id_juez2='$id_juez2',id_juez3='$id_juez3',id_juez4='$id_juez4',id_boxeador1='$id_boxeador1', id_boxeador2='$id_boxeador2',fecha='$fecha', hora='$hora', ganador='$ganador', nombre_foto='$nombre_foto' WHERE id='$id'");
	...
	function mostrarPeleaEstatal() 
	{
		...
		while ($fila = mysqli_fetch_array($resultado)){
				$listado = $listado."<tr><td>".$fila['id']."</td><td><img src='../../resources/images/peleas_estatales/".$fila['nombre_foto']."' width='75' height='75' style= 'border-radius: 50%;'/>"."</td><td>".$fila['categoria']."</td><td>".$fila['division']
